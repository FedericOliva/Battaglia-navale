...
def tuple_to_bytes(data):
    """
    metodo per trasformare tuple in bytes prima di inviarli con l'utilizzo del metodo send
    della libreria socket
    """
    if not isinstance(data, tuple):
        raise Exception("devi passare una tupla alla funzione tuple_to_bytes")
    
    tuple_converted = pickle.dumps(data, -1)
    return tuple_converted

def bytes_to_tuple(data):
    """
    metodo per trasformare i bytes in tuple una volta ricevuti con l'utilizzo del metodo recv
    della libreria socket
    """
    if not isinstance(data, bytes):
        raise Exception("devi passare dei bytes alla funzione bytes_to_tuple")
